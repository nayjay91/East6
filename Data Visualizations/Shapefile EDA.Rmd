...
# do we want to remove certain types of businesses? 
bus_cat_rmv <- c('#N/A', 'LAWN PARK 10 OR MORE SPACES', 'LAWN PARK UNDER 10 SPACES','TAXI VEHICLE', 'TAXI DRIVER') # any more? vending machines, transient merchants? 

#### Categorize Business Data 
food <- c('RESTAURANTS A-M', 'RESTAURANTS N-Z') # 'FOOD VENDING MACHINES', 'FOOD VENDING VEHICLES', 'ITINERANT RESTAURANT', 'MOBILE FOOD TRUCK', 


business_licenses_df %>% 
  filter(!Classifi_1 %in% bus_cat_rmv)
...
census_transformed <- census %>% 
  ...
  mutate(c_area = units::drop_units(st_area(.)))
...
bus_in_district_sf <- st_join(business_licenses_df, city_council_dist2, join = st_intersects) %>% 
  dplyr::select(License_Ye, Business_N, Street_Add, Business_P, Classifica, OBJECTID, Dist, Council_Me) %>% 
  filter(is.na(Council_Me) == FALSE)

# convert from sf to tibble to aggregate 
bus_in_distric_df <- bus_in_district_sf %>% 
  tibble() %>% 
  group_by(Dist, Council_Me, Classifica) %>% 
  summarise(n = n())
...
```{r}
ggplot(data = bus_in_distric_df, aes(x = n, y = Classifica)) +
  geom_col() +
