...
intrsct <- sf::st_intersection(city_council_dist3, census_transformed) %>% 
  ...
  mutate(pop = Population * in_area / c_area, 
         age_u5 = age_u5 * in_area / c_area,
         age_5_9 = age_5_9 * in_area / c_area,
         age_11_14 = age_11_14 * in_area / c_area,
         age_15_17 = age_15_17 * in_area / c_area,
         age_18_24 = age_18_24 * in_area / c_area,
         age_25_34 = age_25_34 * in_area / c_area,
         age_35_44 = age_35_44 * in_area / c_area,
         age_45_54 = age_45_54 * in_area / c_area,
         age_55_64 = age_55_64 * in_area / c_area,
         age_65_74 = age_65_74 * in_area / c_area,
         age_75_84 = age_75_84 * in_area / c_area) %>% 
...
leaflet()  %>%
  addTiles()  %>%
  addCircleMarkers(data = bus_in_district_sf,popup = ~popup,  color = ~pal_business(business_type), stroke = 0, fillOpacity = 1, radius = 4) 
```
...
school_boundaries_sf <- st_join(school_boundaries, city_council_dist2, join = st_intersects) %>% 
  dplyr::select(School, SchoolType, Dist, Council_Me) %>% 
  filter(is.na(Council_Me) == FALSE)

# convert from sf to tibble to aggregate 
school_in_distric_df <- school_boundaries_sf %>% 
  tibble() %>% 
...
```{r}
unique(city_council_dist$Council_Me)
```
