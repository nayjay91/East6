...
```{r}
# Give names to Age
levels(q1$Age) <- c("Under 18","18-24","25-34","35-44","45-54","55-64","65 and older")
...
```{r}
#Can delete this if other plot works
...
ggplot(data = q1_prop, aes(x= as.factor(CFO_Airport_as_a_whole_0_6), y = gender_prop, fill = as.factor(CFO_Airport_as_a_whole_0_6))) +
  geom_col(position = 'dodge') +
  facet_wrap(Gender ~ .) +
  ylab("Proportion (%)") +
  xlab("Satisfaction") + 
...
### 3. Income: 
From the histogram, those with an income of $50,000 to $100,000 had the highet response rate of the survey. Most of the income groups rated the airport as 4 the most.

```{r}
# Give names to Income
levels(q1$Income_code) <- c("Under $50k","$50k-$100k","$100k-$150k","Over 150k","Other")
...
ggplot(data = q1_prop, aes(x= as.factor(CFO_Airport_as_a_whole_0_6), y = income_prop, fill = as.factor(CFO_Airport_as_a_whole_0_6))) +
  geom_col(position = 'dodge') +
...
comments %>%
      ...
      count(sentiment, sort = TRUE)
```
Overall this is not a good result. Depending on how these responses are collected there may be a bias for dissatisfied customers to respond to the survery. We also do no have a baseline for whether airports in general perform very poorly in these sort of surveys, and this is actually a relatively good result, or vice-versa. Regardeless, the sentiment here is overall quite negative. 
...
The plot below visualizes the three different profiles from the customers.
```{r, fig.height=10, fig.width=10}
plot(lcaMod7)
```
The three classes of customers are the following build out three distinct profiles from the customers in relation to signage:
Class 1: Around the age of 35-44, with a salary between $50k-$150k, travelings within the United States, finds the signs and directions in SFO are very good, and the signage on SFO roadways as satisfactory.
Class 2: Young adults (age 18-34) with a a lower salary (under $50k) traveling within the United States find the signs and directions in SFO as very good with the signage of SFO roadways as satisfactory.
Class 3: Adults older than 45 with a salary between $50k-$100k traveling within the United States, find the signs and directions in SFO as excellent while the signage on SFO roadways are very positive. 
