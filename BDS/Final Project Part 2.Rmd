...
Do particular groups of passengers have more trouble with signage (one of the most commented issue areas) than others? i.e can accessibility be improved by identifying and targeting improvements for at risk groups. Possible variables to investigate: "Age", "Gender", "Income_code", "Destination_Geo_Loc"
...
LCA is used with discrete data, such as survey responses since it only works with categorical data. Since the dataset is not made up of one homogenous group, it is better modeled by multiple groups that are characterized by unique response propensities. We then recevie profiles for the different groups.
...
part_b$Gender <- sapply(part_b$Gender, function(x) (as.numeric(factor(x,levels = c(1,2)))))
...
lcaFormula <-  cbind(Age, 
         ...
         Destination_Geo_Loc, 
         Signs_and_directions_inside_SFO_0_6, 
         Signs_and_directsion_on_roadways_0_6) ~ 1
lcaMod2 <-  poLCA(lcaFormula,part_b, nclass = 2, maxiter = 5000)
lcaMod3 <-  poLCA(lcaFormula,part_b, nclass = 3, maxiter = 5000)
lcaMod4 <-  poLCA(lcaFormula,part_b, nclass = 4, maxiter = 5000)
lcaMod5 <-  poLCA(lcaFormula,part_b, nclass = 5, maxiter = 5000)
lcaMod6 <-  poLCA(lcaFormula,part_b, nclass = 6, maxiter = 5000)
```
To determine which is the best model, we compare AIC for statistical criterion. We see that class 3 has the lowest AIC and so is the best model to use.
```{r}
