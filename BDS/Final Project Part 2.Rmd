...
# Give names to Age
levels(q1_rev$Age) <- c("Under 18","18-24","25-34","35-44","45-54","55-64","65 and older")
```
```{r}
q1_prop <- q1_rev %>% 
  group_by(Age) %>% 
...
levels(q1_rev$Gender) <- c("Male","Female")
```
```{r}
q1_prop <- q1_rev %>% 
  group_by(Gender) %>% 
...
3. Income: From the histogram, those with an income of $50,000 to $100,000 had the highet response rate of the survey. Most of the income groups rated the airport as 4 the most.

```{r}
q1_rev <- q1_rev %>% 
  mutate(Income_code = as.factor(Income_code))

# Give names to Income
levels(q1_rev$Income_code) <- c("Under $50k","$50k-$100k","$100k-$150k","Over 150k","Other")
```
```{r}
q1_prop <- q1_rev %>% 
